...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        if (argc != 3)
        {
            cout << ">> Provide the path to the training xml and a name as the arguments" << endl;
            cout << ">> Example:   ./dtc-trainer path/to/xml books" << endl;
            cout << endl;
            return 0;
        }
        const std::string xml_directory = argv[1];
        const std::string svm_name = argv[2];
        ...
        // file.
        load_image_dataset(images_train, face_boxes_train, xml_directory+"/dtc-training.xml");
        load_image_dataset(images_test, face_boxes_test, xml_directory+"/dtc-training.xml");
        ...
        // a face.
        image_window hogwin(draw_fhog(detector), ":: DTC fHOG Detector ::");
        ...
        */
        ...
        // serialize() function.
        serialize(xml_directory+'/'+svm_name+".svm") << detector;

        // Then you can recall it using the deserialize() function.
        object_detector<image_scanner_type> detector2;
        deserialize(xml_directory+svm_name+".svm") >> detector2;
        cout << ">> SVM serialized to -->" << xml_directory+'/'+svm_name+".svm\n" << endl;
